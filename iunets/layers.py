    ...
    *Warning*: Note that this is different from the definition of additive
    ...
    def split(self, x, mode):
        if mode == "split-by-chunk":
            xa = x[:,:self.channel_split_pos,:,:]
            xb = x[:,self.channel_split_pos:,:,:]
    ...
    def forward(self, x):
        ...
        log_det_jac  = torch.log(s).view(n,-1).sum(-1)
...
class StandardBlock(nn.Module):
    def __init__(self,
        ...
        self.num_out_channels = num_out_channels*2

        for i in range(depth):

            current_in_channels = max(self.num_in_channels, self.num_out_channels)
            current_out_channels = max(self.num_in_channels, self.num_out_channels)
            # current_in_channels = self.num_in_channels
            # current_out_channels = self.num_out_channels
...
class ConditionalAffineCoupling(nn.Module):
    ...
    def forward(self, x, temb, jac):
        ...
        # shift = self.F2(temb)
        # print(F"""channel_split_pos: {self.channel_split_pos}
        # scale: {scale.shape}
        # shift: {shift.shape}
        # x: {x.shape}
        # """)
...
# pass data and time embedding through affine coupling layer
# scale and shift data using time embedding (CombinedBlock) layer
# pass scaled and shifted data through affine coupling layer

def create_conditional_module(in_channels, **kwargs):
    ...
    print(f"num_F_in_channels: {num_F_in_channels}")
    print(f"num_F_out_channels: {num_F_out_channels}")
