...
from .layers import ConditionalAffineCoupling
...
class iUNet(nn.Module):
    ...
    def encode(self, x, times, use_padding=False):
        ...
        for i in range(self.num_levels):
            ...
            for j in range(depth):
                encoder_type = self.encoder_modules[i][j]._modules['_fn'].__class__ 
                if encoder_type is ConditionalAffineCoupling:
                    (bs, _, dim, _) = x.shape
                    pos_emb = SinusoidalPosEmb(dim)
                    temb = pos_emb(times).repeat(1,1,dim,1).reshape(bs,1,dim,dim)
                    x, temb, log_det_jac = self.encoder_modules[i][j].forward(x, temb, log_det_jac)
                else:
        ...
        codes.append(x)
        return tuple(codes), log_det_jac


    def decode(self, times, *codes):
        ...
        for i in range(self.num_levels-1, -1, -1):
            ...
            # RevNet R
            for j in range(depth):
                decoder_type = self.decoder_modules[i][j]._modules['_fn'].__class__
                if decoder_type is ConditionalAffineCoupling:
                    (bs, _, dim, _) = x.shape
                    pos_emb = SinusoidalPosEmb(dim)
                    temb = pos_emb(times).repeat(1,1,dim,1).reshape(bs,1,dim,dim)
                    x, temb, log_det_jac = self.decoder_modules[i][j].forward(x, temb, log_det_jac)
                else:
    ...
    def forward(self, x: torch.Tensor, times: torch.Tensor):
        ...
        code, ldj_enc = self.encode(x, times, use_padding=False)
        x, ldj_dec = self.decode(times, *code)
        log_det_jac = ldj_enc + ldj_dec
        
        if self.padding_mode is not None and self.revert_input_padding:
            x = self.revert_padding(x, padding)
        
        return x, times, torch.abs(log_det_jac)
       

    def decoder_inverse(self, x, times, use_padding=False):
        ...
        for i in range(self.num_levels):
            ...
            for j in range(depth-1, -1, -1):
                ...
                if decoder_type is ConditionalAffineCoupling:
                    ...
                    pos_emb = SinusoidalPosEmb(dim)
                    temb = pos_emb(times).repeat(1,1,dim,1).reshape(bs,1,dim,dim)
                    x, temb, log_det_jac = self.decoder_modules[i][j].inverse(x, temb, log_det_jac)
                else:
    ...
    def encoder_inverse(self, *codes, times):
        ...
        for i in range(self.num_levels-1, -1, -1):
            ...
            for j in range(depth-1, -1, -1):
                ...
                if encoder_type is ConditionalAffineCoupling:
                    ...
                    pos_emb = SinusoidalPosEmb(dim)
                    temb = pos_emb(times).repeat(1,1,dim,1).reshape(bs,1,dim,dim)
                    x, temb, log_det_jac  = self.encoder_modules[i][j].inverse(x, temb, log_det_jac)
                else:
    ...
    def inverse(self, x: torch.Tensor, times: torch.Tensor):
        ...
        code, ldj_dec  = self.decoder_inverse(x, times, use_padding=False)
        x, ldj_enc = self.encoder_inverse(*code, times)
        log_det_jac = ldj_dec + ldj_enc
        if self.padding_mode is not None and self.revert_input_padding:
            if self.verbose:
                warnings.warn(
            ...
            x = self.revert_padding(x, padding)
        return x, torch.abs(log_det_jac)
